...
contract SuccinctStakingSlashTests is SuccinctStakingTest {
    ...
    function testFuzz_Slash_VariableAmounts(uint256 _slashAmount) public {
        ...
        assertEq(SuccinctStaking(STAKING).proverStaked(ALICE_PROVER), stakeAmount - actualSlashAmount);
        assertEq(SuccinctStaking(STAKING).staked(STAKER_1), stakeAmount - actualSlashAmount);
    }

    function testFuzz_Slash_MultipleStakers(uint256[3] memory _stakeAmounts, uint256 _slashAmount) public {
        ...
        for (uint i = 0; i < stakers.length; i++) {
            uint256 expectedStake = _stakeAmounts[i] - (_stakeAmounts[i] * actualSlashed / totalStaked);
            assertApproxEqAbs(
    ...
    function testFuzz_Slash_WithDispenseBeforeSlash(uint256 _dispenseAmount, uint256 _slashAmount) public {
        uint256 stakeAmount = STAKER_PROVE_AMOUNT;
        ...
        if (slashAmount >= iPROVEBalance * 90 / 100) {
            assertLe(actualStaked, stakedAfterDispense / 10, "Large slash should leave minimal stake");
        } else {
            // For smaller slashes, ensure the result is reasonable
            // The actual calculation depends on the vault exchange rates
            uint256 minExpected = (stakedAfterDispense * (iPROVEBalance - slashAmount)) / iPROVEBalance / 2;
    ...
    function testFuzz_Slash_MultipleSlashRequests(uint256[3] memory _slashAmounts) public {
        ...
        for (uint i = 0; i < _slashAmounts.length; i++) {
        ...
        for (uint i = slashIndices.length; i > 0; i--) {
