...
abstract contract ProverRegistry is IProverRegistry {
    ...
    /// @inheritdoc IProverRegistry
    function createProver(uint256 _feeBips) external override returns (address) {
        if (hasProver(msg.sender)) {
            revert ProverAlreadyExists();
        }

        return _deployProver(_feeBips);
    }

    /// @dev Uses CREATE2 to deploy an instance of SuccinctProver and adds it to the mapping.
    function _deployProver(uint256 _feeBips) internal returns (address) {
        ...
        address prover = Create2.deploy(
            ...
            abi.encodePacked(
                ...
                abi.encode(iProve, address(this), msg.sender, proverCount, _feeBips)
            )
        );
